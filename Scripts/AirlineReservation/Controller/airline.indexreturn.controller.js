angular.module('app').controller('airline.indexreturn.controller', function ($scope, $http, httpMethodsService) {
    ...
    $scope.flyingDateReturn = new Date();
    ...
    $scope.Search = function () {
        if ($scope.SelectedFlyingFrom == null || $scope.SelectedFlyingTo == null || $scope.flyingDate == null || $scope.SelectedFlyingClass == null || $scope.SelectedFlyingCost == null || $scope.flyingDateReturn == null)
            alert("Please select full value to search!");
        location.href = "/airline/detailsreturn/" + $scope.SelectedFlyingFrom.From
            ...
            + "/" + moment($scope.flyingDateReturn).format("DDMMYYYY");
    }
